...
void StarLayers::create(GLuint program, int quantity) {
  ...
  for (auto &&[index, layer] : iter::enumerate(m_starLayers)) {
    ...
    abcg::glBufferData(GL_ARRAY_BUFFER, data.size() * sizeof(glm::vec3), data.data(), GL_STATIC_DRAW);
    abcg::glBindBuffer(GL_ARRAY_BUFFER, 0);
    abcg::glGenVertexArrays(1, &layer.m_VAO);
    abcg::glBindVertexArray(layer.m_VAO);
    abcg::glBindBuffer(GL_ARRAY_BUFFER, layer.m_VBO);
    abcg::glEnableVertexAttribArray(positionAttribute);
    abcg::glVertexAttribPointer(positionAttribute, 2, GL_FLOAT, GL_FALSE, sizeof(glm::vec3) * 2, nullptr);
    abcg::glEnableVertexAttribArray(colorAttribute);
    abcg::glVertexAttribPointer(colorAttribute, 3, GL_FLOAT, GL_FALSE, sizeof(glm::vec3) * 2, reinterpret_cast<void *>(sizeof(glm::vec3)));
    abcg::glBindBuffer(GL_ARRAY_BUFFER, 0);
  ...
  for (auto const &layer : m_starLayers) {
    ...
    for (auto const i : {-2, 0, 2}) {
      for (auto const j : {-2, 0, 2}) {
        abcg::glUniform2f(m_translationLoc, layer.m_translation.x + j, layer.m_translation.y + i);
